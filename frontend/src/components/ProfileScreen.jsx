...
export default function ProfileScreen({ lang, onSelect, api }) {
  const t = LANGS[lang]
  ...
  return (
    <div className="screen" style={{ background: '#F4F1FF', gap: 0 }} data-screen-label="02 Profile Select">
      <div style={{ fontSize: 28, fontWeight: 800, color: '#1E1A33', marginBottom: 8, textAlign: 'center' }}>
        {t.choose}
      </div>
      <div style={{ fontSize: 16, color: '#8A85A5', fontWeight: 600, marginBottom: 40 }}>EN · FR · KIN</div>
      ...
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', justifyContent: 'center' }}>
        {profiles.map((p) => (
          ...
          >
            <div
              style={{
                width: 80, height: 80, borderRadius: '50%', background: p.color,
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: '#1E1A33' }}>{p.name}</div>
            <div style={{ display: 'flex', gap: 4 }}>
              {Array.from({ length: 5 }, (_, i) => (
                <svg key={i} width={18} height={18} viewBox="0 0 54 54">
                  <polygon
            ...
            <div style={{ fontSize: 13, color: '#8A85A5', fontWeight: 700 }}>Level {p.level}</div>
          </div>
        ))}
      </div>

      <button className="btn-secondary" style={{ marginTop: 40 }} onClick={() => onSelect(null)}>
        + Add learner
      </button>
    </div>
  )
