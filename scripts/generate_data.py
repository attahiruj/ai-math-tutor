...
OBJECTS_FR = {
    "apples": "pommes", "goats": "chèvres", "stars": "étoiles",
    "fish": "poissons", "fingers": "doigts", "mangoes": "mangues",
    "bananas": "bananes", "cows": "vaches", "eggs": "œufs",
    "beans": "haricots", "drums": "tambours", "beads": "perles",
}

OBJECTS_KIN = {
    "apples": "pome", "goats": "ihene", "stars": "inyenyeri",
    "fish": "ifi", "fingers": "intoki", "mangoes": "imyembe",
    "bananas": "ingizi", "cows": "inka", "eggs": "amagi",
    "beans": "ibishyimbo", "drums": "ingoma", "beads": "urunigi",
}
...
def gen_number_sense() -> list[dict]:
    ...
    for a, b in pairs:
        ...
        idx += 1
    befores = [(47, 49, 48), (14, 16, 15), (29, 31, 30), (8, 10, 9), (19, 21, 20), (5, 7, 6)]
...
def gen_addition() -> list[dict]:
    ...
    idx = 1
    simple = [
        (1, 1, "beads"), (2, 3, "beads"), (3, 4, "beads"),
        (4, 5, "mangoes"), (5, 4, "mangoes"), (6, 3, "mangoes"),
    ]
    ...
    word_probs = [
        ...
        ("Amina scored 8 points. She scores 9 more. Total?",
         ...
         17, "beads_8_plus_9", 6),
        ("27 + 14 equals?", "27 + 14 égale?", "27 + 14 ni angahe?",
         41, "beads_27_plus_14", 8),
        ("36 + 25 equals?", "36 + 25 égale?", "36 + 25 ni angahe?",
         61, "beads_36_plus_25", 9),
    ]
    for en, fr, kin, ans, vis, diff in word_probs:
        items.append({
            ...
            "difficulty": diff,
            "age_band": "7-8" if diff <= 6 else "8-9",
            "stem_en": en, "stem_fr": fr, "stem_kin": kin,
            "visual": vis,
            "answer_int": ans,
            "distractors": make_distractors(ans),
...
def gen_subtraction() -> list[dict]:
    ...
    idx = 1
    simple = [
        (8, 3, "drums"), (10, 4, "drums"), (9, 2, "beans"),
        (7, 5, "beans"), (6, 1, "fingers"), (10, 6, "fingers"),
    ]
    ...
    word_probs = [
        ...
        ("A school has 18 chairs. 9 are taken away. How many remain?",
         ...
         9, "chairs_18_minus_9", 7),
        ("62 - 28 equals?", "62 - 28 egale?", "62 - 28 ni angahe?",
         34, "beads_62_minus_28", 9),
        ("50 - 17 equals?", "50 - 17 egale?", "50 - 17 ni angahe?",
         33, "beads_50_minus_17", 8),
    ]
    for en, fr, kin, ans, vis, diff in word_probs:
        items.append({
            "id": f"S{idx:03d}",
...
def gen_word_problem() -> list[dict]:
    probs = [
        ...
        ("Mandazi costs 50 RWF each. Jean has 400 RWF. How many can he buy?",
         ...
         8, "rwf_400_mandazi_50", 8, "8-9"),
        ("5 birds sit on a tree. 3 more arrive. How many birds total?",
         "5 oiseaux sont sur un arbre. 3 autres arrivent. Combien au total?",
         "Inyoni 5 zirimo ku giti. Izindi 3 zaza. Hamwe ni zingahe?",
         8, "birds_5_plus_3", 6, "7-8"),
    ...
    return [
        {
            ...
            "answer_int": ans,
            "distractors": make_distractors(ans),
            "hint_en": "Read the problem again and draw a picture.",
            "hint_kin": "Soma ikibazo nanone ukore ishusho.",
        }
        for idx, (en, fr, kin, ans, vis, diff, age) in enumerate(probs, 1)
    ]
...
def build_curriculum() -> list[dict]:
    ...
    generated = (
        gen_counting() + gen_number_sense()
        + gen_addition() + gen_subtraction()
        + gen_word_problem()
    )
    gen_ids = {item["id"] for item in generated}
    for s in seed:
        if s["id"] not in gen_ids:
            s.setdefault("stem_fr", s.get("stem_en", ""))
            s.setdefault("stem_kin", s.get("stem_en", ""))
            s.setdefault("distractors", make_distractors(s["answer_int"]))
            s.setdefault("hint_en", "Think carefully.")
            s.setdefault("hint_kin", "Tekereza neza.")
            generated.append(s)
    for item in generated:
        item.setdefault("stem_fr", item.get("stem_en", ""))
        item.setdefault("stem_kin", item.get("stem_en", ""))
        item.setdefault("distractors", make_distractors(item["answer_int"]))
        item.setdefault("hint_en", "Think carefully.")
        item.setdefault("hint_kin", "Tekereza neza.")
    return generated
